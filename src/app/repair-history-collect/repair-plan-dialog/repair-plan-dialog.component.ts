...
export class RepairPlanDialogComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
        ...
        this.form.setValue(
          {
            ...
            date: value.post_date ? value.post_date.toDate() : null,
          }
        );
      }
    });
    ...
    this.number_un = this.form.controls['number'].valueChanges.withLatestFrom(this.$plan_data).subscribe(
      (value: [number, RepairPlanSingleDataInterface[]]) => {
        const _ = [];
        console.log(value);
        if (this.origin_data) {
          if (value[0].toString() !== this.origin_data.number) {
            _.push('您正在修改天窗修编号，请谨慎修改');
            if (value[1].findIndex(value2 => value2.number === value[0].toString()) >= 0) {
